...
function editTTCell(encKey) {
  ...
  modal.innerHTML = `<div class="tt-modal-inner"><h3><i class="ti ti-calendar-week"></i> ${day} · ${slot}</h3><input type="text" id="ttSubjectName" placeholder="Subject name" value="${existing.name||''}"><input type="text" id="ttRoom" placeholder="Room / link" value="${existing.room||''}"><div class="edit-cell-btns"><button class="btn btn-outline btn-sm" style="width:auto" onclick="closeTTModal()">Cancel</button>${existing.name?`<button class="btn btn-sm btn-red" style="width:auto" onclick="clearTTCell('${encodeURIComponent(key)}')"><i class="ti ti-x"></i> Clear</button>`:''}<button class="btn btn-sm" style="width:auto" onclick="saveTTCell('${encodeURIComponent(key)}')">Save <i class="ti ti-check"></i></button></div></div>`;
